...
Job Parameters (set in the Glue job's --job-parameters):
  ...
  --output_file_path        S3 path where the output .pkl file will be saved
                            (e.g. s3://my-bucket/retrievers/wfca_retriever.pkl)
...
output_file_path       = _args["output_file_path"]
...
# Derive the metrics JSON path by swapping the .pkl extension
metrics_path = output_file_path.rsplit(".", 1)[0] + "_metrics.json"
print(f"\nSaving metrics to: {metrics_path}")
save_metrics_to_s3(metrics, metrics_path)
print(json.dumps(metrics, indent=2))

# --- Step 6: Package and save -----------------------------------------------
# Both bundles are stored in a single pkl so downstream inference only needs
# one file to load.
output_payload = {
    "tenant": tenant,
    "built_at": datetime.datetime.utcnow().isoformat(),
...
print(f"\nSaving retriever bundle to: {output_file_path}")
save_bundle_to_s3(output_payload, output_file_path)
